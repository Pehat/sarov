...
var ISOMETRIC_RATIO = 1 / Math.sqrt(2);
var FONT_SIZE = 30;
...
//TODO: consider usage of CanMerge()
function CalcOptim() {
    var s = 0;
    for (var k = 0; k < pivotCount; k++) {
        var i = 0;
        var a = getFieldAt(i, k);
        if (a > 0) {
            var diskInfo = getLabelAndColor(a);
            
            m1 = (a - 1) % TOWER_HEIGHT;
            m2 = Math.floor((a - 1) / TOWER_HEIGHT);
            ...
            i++;
            while ((a = getFieldAt(i, k)) && i < PIVOT_HEIGHT - 1) {
                var nextDiskInfo = getLabelAndColor(a);
                
                if (!((diskInfo[1] == nextDiskInfo[1]) && nextDiskInfo[0] == diskInfo[0] - 1)) {
                    s++;
                }
...
function showTable() {
    ...
    var minDiskWidth = cellWidth * 0.25;
    var maxDiskWidth = cellWidth * 0.8;
    
    context.font = FONT_SIZE + "px Sans-serif";
    context.fillStyle = '#afaf5f';
    ...
    for (var i = PIVOT_HEIGHT; i--; ) {
        for (var j = 0; j < pivotCount; j++) {
            ...
            var fieldValue = getFieldAt(PIVOT_HEIGHT - 1 - i, j);
            
            if (fieldValue > 0) {
                var diskInfo = getLabelAndColor(fieldValue);
                var diskWidth = minDiskWidth + diskInfo[0] / (TOWER_HEIGHT - 1) * (maxDiskWidth - minDiskWidth);
                var diskOffset = (cellWidth - diskWidth) / 2;
                
                //TODO: done=1 if under the cell is sequential growth
                context.fillStyle = palette[diskInfo[1]];
                                
                context.setTransform(1, 0, 0, ISOMETRIC_RATIO, 0, canvas.height - contextHeight * ISOMETRIC_RATIO - cellHeight);
                var sideGradient = context.createLinearGradient(
                    Math.floor((j + 0.5) * cellWidth - diskWidth * 0.5), 
                    Math.floor((i + 1) * cellHeight / ISOMETRIC_RATIO), 
                    Math.floor((j + 0.5) * cellWidth + diskWidth * 0.5), 
                    Math.floor((i + 1) * cellHeight / ISOMETRIC_RATIO)
                );
                sideGradient.addColorStop(0.0, palette[diskInfo[1]]);
                sideGradient.addColorStop(0.2, '#ffffff');
                sideGradient.addColorStop(1.0, palette[diskInfo[1]]);
                context.fillStyle = sideGradient;
                
                context.beginPath();
                context.arc(
                    Math.floor((j + 0.5) * cellWidth),
                    Math.floor((i + 1) * cellHeight / ISOMETRIC_RATIO),
                    Math.floor(diskWidth * 0.5),
                    0, Math.PI
                );
                context.lineTo(
                    Math.floor(j * cellWidth + diskOffset), 
                    Math.floor(i * cellHeight / ISOMETRIC_RATIO)
                );
                context.arc(
                    Math.floor((j + 0.5) * cellWidth),
                    Math.floor(i * cellHeight / ISOMETRIC_RATIO),
                ...
                context.lineTo(
                    Math.floor((j + 1) * cellWidth - diskOffset), 
                    Math.floor((i + 1) * cellHeight / ISOMETRIC_RATIO)
                );
                ...
                gradient.addColorStop(1.0, palette[diskInfo[1]]);
                gradient.addColorStop(0.5, '#ffffff');
                gradient.addColorStop(0.0, palette[diskInfo[1]]);
                ...
                context.setTransform(1, 0, 0, 1, 0, canvas.height - contextHeight * ISOMETRIC_RATIO - cellHeight);
                var bevelGradient = context.createLinearGradient(
                    Math.floor((j + 0.5) * cellWidth - 0.5 * FONT_SIZE), 
                    Math.floor((i + 1) * cellHeight / ISOMETRIC_RATIO), 
                    Math.floor((j + 0.5) * cellWidth + 0.5 * FONT_SIZE), 
                    Math.floor((i + 1) * cellHeight / ISOMETRIC_RATIO)
                );
                ...
                context.lineWidth = 2;
                context.strokeText(diskInfo[0], -1 + (j + 0.5) * cellWidth, 1 + (i + 0.5) * cellHeight + 0.5 * diskWidth * ISOMETRIC_RATIO);
                context.strokeStyle = 'white';
                context.strokeText(diskInfo[0], 1 + (j + 0.5) * cellWidth, -1 + (i + 0.5) * cellHeight + 0.5 * diskWidth * ISOMETRIC_RATIO);
                context.fillStyle = palette[diskInfo[1]];
                context.fillText(diskInfo[0], (j + 0.5) * cellWidth, (i + 0.5) * cellHeight + 0.5 * diskWidth * ISOMETRIC_RATIO);
