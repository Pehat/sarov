...
function renderTrapezoid(lowerDiskInfo, upperDiskInfo, left, top) {
    ...
    //TODO: simplify
    var diskWidthTop = minDiskWidth + upperDiskInfo[0] / TOWER_HEIGHT * disksWidthRange;
    ...
    var diskWidthBottom = minDiskWidth + (lowerDiskInfo[0] + 1) / TOWER_HEIGHT * disksWidthRange;
...
function animateUp(timestamp) {
    ...
    var y = stackY - (stackY + cellHeight * (stackHeight - 1)) * Math.min((timestamp - animationTimeStart) / ANIMATION_TIME, 1);
    renderTrapezoid(stackBottomInfo, stackTopInfo, stackX, y);
...
function animateSide(timestamp) {
    ...
    var x = stackX - (stackX - stackEndCol * cellWidth) * Math.min((timestamp - animationTimeStart) / ANIMATION_TIME, 1);
    renderTrapezoid(stackBottomInfo, stackTopInfo, x, -cellHeight * (stackHeight - 1));
...
function animateDown(timestamp) {
    ...
    var y = -cellHeight * (stackHeight - 1) - (cellHeight * (stackEndRow - 1 - stackHeight)) * Math.min((timestamp - animationTimeStart) / ANIMATION_TIME, 1);
    renderTrapezoid(stackBottomInfo, stackTopInfo, stackEndCol * cellWidth, y);
...
function showTable() {
    ...
    cellHeight = contextHeight / PIVOT_HEIGHT;
    
    cellHeight = cellWidth * 0.3;
    
    minDiskWidth = cellWidth * 0;
    maxDiskWidth = cellWidth * 0.8;
    ...
    for (var j = 0; j < pivotCount; j++) {    
        ...
        for (var i = PIVOT_HEIGHT; i--; ) {            
            ...
            if (!CanMerge(diskInfo, upperDiskInfo)) {
                renderTrapezoid(lowerDiskInfo, upperDiskInfo, j * cellWidth, i * cellHeight);
...
function UpCell(col) {
    ...
    if (upperDisk >= 0) {
        ...
        //TODO: FIX
        stackX = stackCol * cellWidth;
        stackY = (PIVOT_HEIGHT - 2 - upperDisk) * cellHeight;
        stackHeight = h;
...
function DownCell(col) {
    ...
    stackEndCol = (canput) ? col : stackCol;
...
function EndDownCell() {
    ...
    if (stackEndCol != stackCol) {
        movesHistory.push([stackCol, stackEndCol, stackHeight]);
        scoresHistory.push(stackCol + stackEndCol * 12);
        ++moveNumber;
    }
